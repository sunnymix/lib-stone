...
@Reviser public class ObjOptimizer {
    @Reviser public static class ClassStateEx extends ClassState {
        ...
        public Object eval(Environment env) {
            ...
            ArrayList<DefState> methods = new ArrayList<DefState>();
    ...
    @Reviser public static class ClassBodyEx extends ClassBody {
        ...
        public Object eval(Environment env) {
            for (AstTree t: this)
                if (!(t instanceof DefState))
                    ((AstTreeEx)t).eval(env);
            return null;
        }
        public void lookup(Symbols syms, Symbols methodNames,
                           Symbols fieldNames, ArrayList<DefState> methods)
        {
            for (AstTree t: this) {
                if (t instanceof DefState) {
                    DefState def = (DefState)t;
                    ...
                    if (i >= oldSize)
                        ...
                        methods.set(i, def);
                    ((DefStateEx2)def).lookupAsMethod(fieldNames);
                }
                else
    ...
    @Reviser public static class DefStateEx2 extends EnvOptimizer.DefStateEx {
